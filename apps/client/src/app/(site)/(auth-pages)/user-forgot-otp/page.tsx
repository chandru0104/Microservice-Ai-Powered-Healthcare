...
import { UserForgotOtp } from 'apps/client/src/services/authService';
...
const UserForgot: React.FC = () => {
    ...
    const onFinish = async (values: { otp: string }) => {
        try {
            setLoading(true)
            const temEmail = localStorage.getItem("forgotTempEmail")
            if (!temEmail) {
                throw new Error("Please provid email ")
            }


            const verify = await UserForgotOtp({ email: temEmail, otp: values.otp })
            if (verify) {
