...
import { DoctorForgotOtp } from 'apps/client/src/services/authService';
...
const DoctorForgot: React.FC = () => {
    ...
    const onFinish = async (values: { otp: string }) => {
        try {
            setLoading(true)
            const temEmail = localStorage.getItem("forgotTempEmailDoctor")
            if (!temEmail) {
                throw new Error("Please provid email ")
            }


            const verify = await DoctorForgotOtp({ email: temEmail, otp: values.otp })
            if (verify) {
