...
import { DoctorForgotEmail } from "../../../../services/authService"
import { useRouter } from 'next/navigation';

const sendEmailDoctor: React.FC = () => {
    ...
    const onFinish = async (values: { email: string }) => {
        try {
            setLoading(true)
            const sendEmail = await DoctorForgotEmail(values)
            if (sendEmail) {
                if (typeof window === "object") {
                    localStorage.setItem("forgotTempEmailDoctor", values.email)
                }
                router.push("/doctor-forgot-otp")
            }
